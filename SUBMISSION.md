...
| 6 | Finding reports | Done | One list across everyone the viewer may see: title search, filters for status, owner and assigned approver, sorting by submitted date, status or total, and pagination showing the match count. All of it is SQL — `count(*) over ()` returns the page and the total in one round trip. Sorting by status follows the lifecycle rather than the alphabet. |
| 7 | Acting on many reports at once | Done | Bulk approve and reject, each report checked individually by `canTransition`, with a per-report result naming any refused because the approver owned them while the rest still go through. Plus a CSV of reimbursements due (approved, not yet paid), guarded by `requireApprover`. |
| 8 | Dashboard | Done | Four headline numbers, breakdowns by status and by category, and reimbursements paid per week over eight weeks. The two "this week" counts come from the append-only timeline rather than from `decided_at`/`paid_at`, because a row only remembers its most recent transition — a report approved last week and paid this week would otherwise be miscounted. Scope follows visibility: an approver sees the company, an employee sees themselves. |
...
| 10 | Stale-approval alerts | Done | Reports waiting longer than `STALE_AFTER_DAYS` appear in `/alerts` with a count badge in the navigation. An approver can dismiss one **assigned to them**; dismissals are per person, so silencing an alert does not silence it for the rest of the queue. The alert returns after `ALERT_SNOOZE_DAYS` with no scheduled job — verified by ageing a dismissal past the window and watching the alert reappear on the next page load. |
